...
const JS_FILE = path.join(__dirname, '..', 'js', 'translations.js');

// Translations to apply (key = "lang.longDescXxx", value = correct translated content)
const TRANSLATIONS = {
    // Kunming - 6 languages
    ...
    'ko.longDescShenzhen': '<p>선전(深圳)은 세계에서最も눈부신 도시変신입니다—1980년에 3만 명의 어촌이었다 곳이 오늘날 1,700만 명의 대도시로発展했습니다. 중국 최초의 경제특구로서 개혁개방을 선도했으며 오늘날 지구상에서最も미래적인 스카이라인을誇っています.</p><p>선전은 테크愛好자의 천국으로 화웨이, 텐센트, BYD 등의 주요 기업이 본부를 두고 있습니다.</p><h3>주요 명소</h3><ul><li><strong>월드 윈도우(世界之窗)</strong> — 세계 명소의 미니어처.</li><li><strong>선전 박물관(深圳博物馆)</strong> — 도시의 extraordinary한変身 전시.</li><li><strong>다펜 유화촌(大芬油画村)</strong> — 세계 최대의 유화 복제 생산지.</li><li><strong>텐센트 본사(Mango Valley)</strong> — 자하 하디드가設計した stunning한 캠퍼스.</li></ul><h3>실용 정보</h3><p>선전은 지하철이 매우 잘 되어 있습니다. 홍콩과 함께하는 주말 여행에 최적입니다.</p>',
...
// For each translation, find and replace the value in the JS file
// Using a regex approach: find "lang.longDescXxx" = "old", replace with correct value
let applied = 0;
for (const [key, newVal] of Object.entries(TRANSLATIONS)) {
    const [lang, jsKey] = key.split('.');
    
    // Find the language section first
    const langRe = new RegExp(`"${lang}": \\{`);
    const langMatch = langRe.exec(content);
    if (!langMatch) {
        console.log(`WARNING: ${lang} section not found`);
        continue;
    }
    
    // Find the specific key within that section
    // The structure is: "jsKey": "value",
    // We need to find the key and replace its value
    const keyRe = new RegExp(`"${jsKey}": "([^"]*)"(,\\s*\\n|\\s*\\n\\s*\\})`);
    const keyMatch = keyRe.exec(content);
    if (!keyMatch) {
        console.log(`WARNING: ${key} not found`);
        continue;
    }
    
    // Check if the current value is the wrong (Chinese) one or already correct
    const currentVal = keyMatch[1];
    const isWrong = /[\u4e00-\u9fff]/.test(currentVal) && 
        (currentVal.includes('昆明') || currentVal.includes('广州') || 
         currentVal.includes('深圳') || currentVal.includes('戏曲') || 
         currentVal.includes('京剧'));
    
    if (!isWrong && currentVal === newVal) {
        console.log(`SKIP: ${key} already correct`);
        continue;
    }
    
    // Replace the value
    const oldVal = keyMatch[1];
    // Use a direct string replace
    const before = content.substring(0, keyMatch.index);
    const after = content.substring(keyMatch.index);
    const replaceRe = new RegExp(`"${jsKey}": "${oldVal.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"(,\\s*\\n|\\s*\\n\\s*\\})`);
    const newContent = before + after.replace(replaceRe, `"${jsKey}": "${newVal}"$1`);
    
    if (newContent === content) {
        console.log(`WARNING: Replace failed for ${key}`);
    } else {
        content = newContent;
        applied++;
        console.log(`OK: ${key}`);
...
console.log('Wrote', JS_FILE);

// Syntax check
const { execSync } = require('child_process');
try {
    execSync(`node --check "${JS_FILE}"`, { encoding: 'utf8' });
    console.log('Syntax check: PASSED');
} catch (e) {
    console.log('Syntax check FAILED:', e.stderr || e.message);
    process.exit(1);
}
