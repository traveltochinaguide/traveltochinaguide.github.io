...
// Extract the original carouselData and cityDetails
const carouselData = sandbox.carouselData;
const cityDetails = sandbox.cityDetails;

// If we couldn't extract, exit
if (typeof carouselData === 'undefined' || typeof cityDetails === 'undefined') {
  console.error('Could not extract carouselData or cityDetails from the sandbox.');
  process.exit(1);
}
...
const translationsObject = `const translations = ${JSON.stringify(translations, null, 2)};`;
const carouselDataString = `const carouselData = ${JSON.stringify(carouselData, null, 2)};`;
const cityDetailsString = `const cityDetails = ${JSON.stringify(cityDetails, null, 2)};`;
