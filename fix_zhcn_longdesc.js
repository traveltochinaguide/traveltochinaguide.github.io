...
const filePath = './js/translations.js';
...
let braceCount = 1;
let i = startIndex + startToken.length; // this is the position after the opening brace
let endIndex = -1;
...
for (const city of cities) {
  ...
  const htmlFilePath = `zh-CN/${city}.html`;
  if (!fs.existsSync(htmlFilePath)) {
    console.log(`Warning: File not found: ${htmlFilePath}`);
    continue;
  }
  try {
    const htmlContent = fs.readFileSync(htmlFilePath, 'utf8');
    const match = htmlContent.match(/window\.translations = ({[^}]+});/s);
    if (!match) {
      console.log(`Warning: Could not find window.translations in ${htmlFilePath}`);
      continue;
    }
    const htmlObjStr = match[1];
    const htmlObj = new Function(`return ${htmlObjStr}`)();
    const zhCnSection = htmlObj['zh-CN'];
    if (!zhCnSection) {
      console.log(`Warning: No zh-CN section in ${htmlFilePath}`);
      continue;
    }
    const value = zhCnSection[key];
    if (value === undefined) {
      console.log(`Warning: Key ${key} not found in zh-CN section of ${htmlFilePath}`);
      continue;
    }
    console.log(`Updating ${key} from ${htmlFilePath} (length: ${value.length})`);
    // Update the translations object
    translationsObj['zh-CN'][key] = value;
  } catch (e) {
    console.log(`Error processing ${htmlFilePath}: ${e.message}`);
  }
}
