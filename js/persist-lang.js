(function () {
  ...
  function getPreferredLang() {
    // 1. Check HTML lang attribute (Static Generation Support)
    // If the page itself declares a specific language (other than default 'en'), trust it.
    const htmlLang = document.documentElement.lang;
    if (htmlLang && htmlLang !== 'en') return htmlLang;

    // 2. URL Param (Dynamic Support)
    try {
  ...
  // Only run dynamic link interception if we are on the Root/English site (or if html lang is not set/en)
  const isDefaultLang = (!document.documentElement.lang || document.documentElement.lang === 'en');

  if (isDefaultLang) {
    // delegated click handler for nav links to preserve current language (Dynamic Mode)
    document.addEventListener('click', function (e) {
      ...
      if (lang === 'en') return; // Don't add param for default English
    ...
    // also update anchor hrefs in the nav
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      // also ensure nav labels reflect chosen language
      try { setTimeout(setNavLabels, 50); setTimeout(setNavLabels, 350); } catch (e) { }
    });
  } else {
    // For Static Language Pages (e.g. /zh-CN/), simplified logic:
    // Just ensure nav labels are correct (redundant if static generated, but safe for client-side shifts)
    document.addEventListener('DOMContentLoaded', function () {
      try { setNavLabels(); } catch (e) { }
    });
  }

  // set navigation labels for common keys
  function setNavLabels() {
    ...
    const navMap = {
      ...
      'fr': {
        navHome: 'Accueil', navCities: 'Villes populaires', navNature: 'Nature', navCulture: 'Culture', cityBeijing: 'Pékin', cityShanghai: 'Shanghai', cityXian: 'Xi'an', cityGuilin: 'Guilin', cultureIching: 'I Ching', natureZhangjiajie: 'Zhangjiajie', natureJiuzhaigou: 'Jiuzhaigou', natureYangtze: 'Yangtze' },
