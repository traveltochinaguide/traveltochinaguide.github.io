...
def insert_keys_in_language_section(content, lang, keys_to_add):
    ...
    # We want to insert just before the closing brace.
    # But we need to add a comma before the new keys if there is already content.
    # Check the character before the closing brace: if it's not a newline or space, we might need to add a newline and indentation.
    # For simplicity, we'll insert a newline, then the keys with proper indentation, then a newline, then the closing brace.
    # We'll look at the line before the closing brace to infer indentation.
    ...
    # We'll look at the first line of inner_content that is not empty.
    ...
    for key, value in keys_to_add.items():
        ...
        # But note: the value is already a string that we are going to put inside quotes in the JS object.
        # We must escape any existing quotes and backslashes.
